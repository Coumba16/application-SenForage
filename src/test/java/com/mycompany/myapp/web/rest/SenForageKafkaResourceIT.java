...
class SenForageKafkaResourceIT {
    ...
    void setup() {
        ...
        SenForageKafkaResource kafkaResource = new SenForageKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/sen-forage-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/sen-forage-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
